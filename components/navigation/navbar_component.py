...
class NavbarComponent(BaseComponent):
    ...
    @allure.step('Checking navbar component')
    def check_visible(self, username: str):
        with allure.step(f'Check that navbar component is visible and has text "UI Course"'):
            self.app_title.check_visible()
            self.app_title.check_have_text('UI Course')

        with allure.step(f'Check that welcome text is visible and has text "Welcome, {username}"'):
